...
build_overall_table_html <- function(df, rank_offset = 0) {
  ...
  paste0(
    '<table>\n<thead><tr><th>#</th><th>Player</th><th>Pos</th><th>Pts/Wk</th><th>VBD</th><th>ADP</th><th>+/-ADP</th><th>Tier</th><th>PTier</th></tr></thead>\n<tbody>\n',
    paste(rows, collapse = "\n"),
    "\n</tbody>\n</table>\n"
  )
}
...
build_overall_sheet <- function(df, sort_by, title, out_file) {
  df <- switch(sort_by,
    vbd = df |> arrange(desc(VBD)),
    adp = df |> arrange(is.na(adp), adp)
  ) |>
...
build_overall_sheet(vbd, "vbd", "SFB16 Overall Draft Sheet — VBD order",
                    file.path(OUT_DIR, "draft_sheet_overall_vbd.html"))
build_overall_sheet(vbd, "adp", "SFB16 Overall Draft Sheet — ADP order",
                    file.path(OUT_DIR, "draft_sheet_overall_adp.html"))
